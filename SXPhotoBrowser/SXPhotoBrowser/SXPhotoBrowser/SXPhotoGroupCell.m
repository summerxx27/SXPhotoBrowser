...
@implementation SXPhotoGroupCell
...
- (void)setItem:(SXPhotoGroupItem *)item {
    if (_item == item) {
        return;
    }
    _item = item;
    ...
    @weakify(self);
    [self.imageView yy_setImageWithURL:item.largeImageURL placeholder:item.thumbImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self);
        if (!self) return;
