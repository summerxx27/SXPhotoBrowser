...
#import "SXPhotoGroupCell.h"
#import "CALayer+SXCategory.h"
#import "UIScrollView+Category.h"
#import "UIImage+Category.h"
#import "NSString+Category.h"
#import "YYImageCoder.h"
#import "SXPhotoBrowerMacro.h"
#import "YYWebImageManager.h"
#import "UIImageView+YYWebImage.h"
#define kPadding 20
#define kHiColor [UIColor colorWithRGBHex:0x2dd6b8]

@implementation SXPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((SXPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    _snapshotImage = [_toContainerView sx_snapshotImageAfterScreenUpdates:NO];
    BOOL fromViewHidden = fromView.hidden;
    fromView.hidden = YES;
    _snapshorImageHideFromView = [_toContainerView sx_snapshotImage];
    ...
    SXPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    SXPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    SXPhotoGroupCell *cell = [self cellForPage:currentPage];
    SXPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(SXPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            SXPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                SXPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (SXPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (SXPhotoGroupCell *)dequeueReusableCell {
    SXPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (SXPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (SXPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)showHUD:(NSString *)msg {
    ...
    label.size = CGSizeMake(ceil(size.width * scale) / scale, ceil(size.height * scale) / scale);
    label.font = font;
    label.text = msg;
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    SXPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
